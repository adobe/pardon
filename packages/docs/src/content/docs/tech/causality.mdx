...
Pardon's tracking module is based on node's `async_hooks` API, which has been "experimental" since it was introducted 
in Node 8:
...
The risk is offset by how powerful this paradigm can be for asynchronous programming.

## Concurrent Serial Tests

Other http frameworks provide a single thread of execution: a global
environment object can be used to configure each request and response in turn.

With Pardon, we offer the same ergonomics of a "global object" (`environment`), but the values assigned
to this object are independent of the execution order: only _awaited_ assignments are visible.

The global `environment` object is available in some Pardon-enabled runtimes:
when run with `--run-script` and via the testcase runner.
...
`awaited()` returns the tracked values in `awaited` order,
despite the fact that the actually tracking calls might have been executed the reverse order
(according to the wall clock) due to the `randomDelay()s`.
...
unstable order of asynchronous execution.

We can use this to track assignments to a global environment value.
We also track dependent requests that might be executed via script helpers to
surface dependent requests in `favor`.  Creating an execution graph of what
depends on what.

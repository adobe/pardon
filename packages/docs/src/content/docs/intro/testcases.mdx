...
Trial functions can be implemented by stitching together one or more [flows](./flows),
along with any other validation.

Trials can be selected by name or glob pattern for debugging single trials, and subsets
can be selected according to various criteria, for quicker/light-weight smoke testing
of a few cases.

A basic test could simply run a [flow](./flows.mdx).

```ts title="./health-check.test.ts"
import { trial, flow } from 'pardon/testing';

trial("%env/health-check", ({ env }) => {
  flow({ env })`
    >>>
...
This validates that the `/health-check` endpoint returns 200, parameterized
by the environment.
...
Trials can be organized in _surveys_ (similar to how `it("should", ...)` or `test(...)`  tests from other
frameworks can be organized with `describe` or `suite` blocks).

You can use surveys to organize trials:
```ts
survey("%env", {
  trial("health-check", ({ env }) => {
    ...
  });
})
```
...
```
  ...
  <TabItem label="flows/example-ping.flow.https">
...
const { ingress } = pardon({ env, port })`https://example.com/ping`();
... match ingress against the responses and assert at least one matches ...
```
...
```http title="flows/ping.flow.https" ins=/(?<=<<< )pong/
context:
  - env
  - port?
>>>
...
for when they become ready.

```http ins=/(?<=<<< )check/ ins=">>> check / 25" del=/(?<=<<< |>>> )done.*/ {1-7,9-17}
>>>
...
>>> check / 25
GET https://example.com/task/{task}
...
This sequence starts with a `POST` request, captures the `task` identifier in the response, and proceeds
...
If the response labeled `done` is matched, this ends the sequence because there's no request with that label.

### Using sequences in sequences

Each sequence is run sequentially, but they can rely on any number of sequences to run before,
which are run in parallel with each other.
...
```http title="flows/find-product.flow.https"
context:
  - env
  - port?
  - name
...
```http title="flows/order-pens-and-pencils.flow.https"
context:
  - env
  - port?
use:
  - flow: find-product.flow
    context:
      - env
      - port?
      - name = 'pencils'
...
Pardon runs the two `find-product.flow` calls in parallel.

This is just one option of many for connecting requests.
{/* there may be too many options, the test/automation systems haven't quite come together yet. */}

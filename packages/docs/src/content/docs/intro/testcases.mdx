...
:::note
These docs are stale and need a refresh.

- Split test api (gamut/trials) docs from testcase enumeration framework.
- Split flow documtation into own document.
- Highlight testcase enumeration supports async.
:::

## Tests

Tests build on top of collections and use additional "flow" https files.
These are a similar format but used as scripts for running a flow of request/responses,
...
These files 
 - define the trials/flows to run, and

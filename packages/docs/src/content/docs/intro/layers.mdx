...
In pardon, collections are organized by service directories, 
and the endpoints are organized within these directories.

The design is intended to invite using version control / sharing,
but that, in turn, limits what kind of specifics can go into a
collection.

For instance, where to load credentials for the same services differs
across teams, since each team will have access to different secrets.
...
Collections may define services or provide team-shared configuration.
...
```
  ...
  With this `env=local` will configure any call
...
```http title="extension/example/service.yaml" ins={1-6}
config:
  origin:
    env:
      stage: ...

...
"Did you remember your content-type header?".  Of course you did,
you were focused on the data.  Pardon takes care of the syntax part
of the request so you can focus on the semantics.
...
In this tutorial, changing tabs updates the request: you will need
to run each request before the following one.
...
See pardon also supplies the necessary but forgettable `content-type` header here.
...
<Tabs>
  ...
  <TabItem label="auth">
    ...
    - Notice how Pardon adds
      the password (use the lock to toggle showing the value).

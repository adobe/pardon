...
values `"GET"` and `"https://example.com"`.  
...
By adding `env=stage` into the request we can rewrite the request to use stage.
...
```
    ...
    <ExerciseItem label='?'
      ...
      requiring any changes to the templates.
...
```
Notice how this overwrites the origin in the input with the
correct one for the stage environment.  
    </ExerciseItem>
  </Exercises>
</PardonPlayground>

...
Some parts of templates represent and break down "simple" values like numbers, strings, booleans, or `null`, while
...
Pardon provides a string-interpolation syntax and a javascript/typescript syntax for templating,
the javascript syntax is a little nicer in JSON contexts like request bodies, while the interpolation syntax
is necessary for other parts of the http request, like the request origin, pathname, query params and headers.

### Interpolation Variables

Interpolation templates are surrounded by `{{` and `}}`.  Inside we have the following pieces, _all of which are optional._
...
<Tabs>
  ...
  <TabItem label="hints">
    ...
    to allow a value to match multiple path segments), and `-` removes a value from rendering entirely.
  </TabItem>
  <TabItem label="expr">
    <FlexyCodeGrid centered>
...
:::caution
Other HTTP frameworks might support recursive evaluation of templates, e.g., something like `{{service.origin.{{env}}}}/path/...`,
where the collection defines `{{service.origin.stage}}` and `{{service.origin.prod}}`, along with a suitable definition of `{{env}}`.

Pardon's other mechanisms, such as `config` and `defaults` in the service and endpoint configurations provide this use-case without
sacrificing the readability of the templates.
:::
...
Pardon supports rendering data in scopes.  Besides the root scope, additional
scopes are introduced inside elements of indeterminate arrays or objects.
...
there could be zero, one, or more `key` and `value`s produced.  However
`key` and `value` variables referenced from within each array element
will be defined.
...
definition of `id`, `task` or `done`.

To work around this, we use an indeterminate array, which can merged with
any number of items...
...
:::note
Why not `todos[].id` instead?

Since Pardon is leveraging the typescript compiler for processing these
templates, we would have to boil the ocean to "improve" on this syntax.
:::

### Mapped Data

As a user of this data, you might not want to have to search the array for 
a particular `id`: it would be more natural to bind this as a map.

Pardon supports this with a little syntax: `{ id: key } * [...template]`.
The first part is a template which is applied to each item to _resolve_ a value for `key`
and then the `todos` array is represented internally as a map based on that key.
...
In the above example, we omitted the `id: todos.id` field since it's already
represented.

This can also be used in reverse, although in this case
we need to add `todos.$key` to render the `id` field back:
...
For this usage, we omit the `...` example element and only use the `key` mapping
...
With this variant, the values for `hello` and `hi` are bound to the root scope.
...
:::note
Usually services require the `Content-Type` to be set appropriately, so `[body]` would not be necessary.
:::

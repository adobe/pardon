...
## Optional - Example todo service

The pardon source contains a simple server:
<FileTree>
...
```text ins="todo=T1001" ins="done=true"
todo=T1001
done=true
...
Try again, but with `done=false` to mark the todo action as pending again.
...
Manually changing `done` every time we want to toggle something is great,
but what if we could automatically compute the `done` for each request?

For this next experiment, instead of setting the `done` value, try `do=toggle`
...
this will automatically produce the opposite value of done each time this request
is rendered.
...
export async function isDone({ origin, todo }: TodoOptions) {
  const { ingress } = await pardon({
    origin,
    todo,
  })`GET https://todo.example.com/todos/{{todo}}`();

  return ingress.values.done;
}
```

The operative files here are the `get.https` endpoint, where `done` is parsed
from the response, and `toggle.mix.https` for how `getDone` is used in a template.

Making successive calls using this configuration can now toggle the todo's `done` value.

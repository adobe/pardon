...
description: template scripting
next: false
prev: false
---
...
| `ref.$meld` | same as `ref.$hint('~') -- merges refs in keyed/multivalue contexts |
</div>
...
Adding a regex switches the transform to the inline syntax, because regexes can only validate simple values.
<div class="[&_td]:whitespace-nowrap [&_td]:!align-middle">
| Syntax | Transformation |
| --- | --- |
| `ref % /regex/` | `{{ ref % /regex/ }}` |
...
should result in the same schema as `{ a: b = 10 }`.
...
but it's a tradeoff to allow very big numbers and arbitrary precission decimals to get reserialized as-is and
not mangled by the `String(Number(original))` processing implicit in basic JSON parsing/serialization.

...
description: all the magic variables
next: false
prev: false
---
...
#### The `resolve` meta header

The `[resolve]` meta header specifies the DNS name or IP address to connect to.
This overrides DNS only, the host header and SNI will still be based on the origin of the request.
This mimics `curl`'s `--resolve` option.

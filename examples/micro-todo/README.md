...
1. validates the caller's token against **identity** (`POST /validate` →
   username), then
...
- **standalone** — pointed straight at the services (defaults to
  `localhost:4001/4002`)
...
To test using recordings/... (e.g., todo-lifecycle.log.https), run the server
with
...
then you can run
`pardon-runner ./tests/contract.test.ts --mode=vcr env=local --all`

To test using the recordings/mocks.

To rebuild the recordings or create more, you'll also need the actual backends
started. This is as easy as running `docker compose up -d task identity`.
